...
function dbem_reset_options() {
		global $wpdb;
		$dbem = $wpdb->get_row("SELECT * FROM wp_7_options WHERE option_name = 'dbem'");
		$myopts = maybe_unserialize($dbem->option_value);
		$myopts['dbem_rss_main_title'] = "Fashion Events";
		$myopts['dbem_rss_main_description'] = "The Daily Globe Fashion Events";
		$myopts['events_page'] = 3;
		$newopts = array();
		foreach ( $myopts as $key => $value ) {
			$newopts[preg_replace( '/dbem_/', '', $key)] = $value;
		}
		update_option( 'dbem', $newopts );
		echo "<pre>";
		print_r($newopts);
		echo "</pre>";
}
