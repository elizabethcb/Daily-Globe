	...
	if (!class_exists("NewsTweetClass")) {
		class NewsTweetClass {
			...
			########################################################################
					...
					$cleansearcharray = explode(", ", get_bloginfo('name'));
					$cleansearchterm = $cleansearcharray[0] . '%20' . $category;
					//echo '<pre>';
...
<script type="text/javascript">
			jQuery(document).ready(function(){
				...
				NewsTweetSearchterm = '<?php echo $cleansearchterm; ?>';
...
<div id="newstweet-container">
	<div id="newstweet-header">Realtime results for <em><?php echo(stripslashes($cleansearcharray[0] . ' ' . $category));?></em> on Twitter:</div>
	<div id="twitterwrapper">
	</div>
	<div class="newstweet-clear"></div>
	<!--
	
	<p class="newstweet-viewall"><a href='http://search.twitter.com/search?q=<?php// echo(urlencode(stripslashes($cleansearchterm)));?>' target="_blank">View All</a>
		<?php// if($options['tweetit'] == 'yes'){ echo '<a href="http://twitter.com/home?status='.$cleansearchterm.'" target="_blank">Tweet It</a>'; } ?>
	</p>
	-->
	<div class="newstweet-clear"></div>
</div>
