...
function dbem_events_subpanel() {
	...
	$dbem_opts = get_option('dbem');
	// Disable Hello to new user if requested
	if (isset ( $_GET ['disable_hello_to_user'] ) && $_GET ['disable_hello_to_user'] == 'true') {
		update_option ( 'dbem_hello_to_user', 0 );
		$dbem_opts->dbem_hello_to_user = 0;
	}
...
function dbem_admin_css() {
	...
	. DBEM_URI . 'css/admin.css" type="text/css" />';
	echo $css;
}
...
<h2><?php
	...
	$link = array ();
	$link ['past'] = "<a href='" . get_bloginfo ( 'url' ) . "/wp-admin/admin.php?page=events-manager/events-manager.php&amp;scope=past&amp;order=desc'>" . __ ( 'Past events', 'dbem' ) . "</a>";
	$link ['all'] = " <a href='" . get_bloginfo ( 'url' ) . "/wp-admin/admin.php?page=events-manager/events-manager.php&amp;scope=all&amp;order=desc'>" . __ ( 'All events', 'dbem' ) . "</a>";
	$link ['future'] = "  <a href='" . get_bloginfo ( 'url' ) . "/wp-admin/admin.php?page=events-manager/events-manager.php&amp;scope=future'>" . __ ( 'Future events', 'dbem' ) . "</a>";
...
<ul class="subsubsub">
	<li><a href='admin.php' class="current"><?php
	_e ( 'Total', 'dbem' );
	?> <span
		class="count">(<?php
	echo (count ( $events ));
	...
	<tbody>
	  ...
	  <tr <?php
			...
			<td><strong><a class="row-title"
				href="<?php
			bloginfo ( 'url' )?>/wp-admin/admin.php?page=events-manager/events-manager.php&amp;action=edit_event&amp;event_id=<?php
			echo $event ['event_id'];
			?>"><?php
			...
			<td>
 	    	...
 	    	<a
				href="<?php
			bloginfo ( 'url' )?>/wp-admin/admin.php?page=events-manager/events-manager.php&amp;action=duplicate_event&amp;event_id=<?php
			echo $event ['event_id'];
			?>"
				title="<?php
  	     ...
  	     <?php
			...
			<a
				href="<?php
				bloginfo ( 'url' )?>/wp-admin/admin.php?page=events-manager/events-manager.php&amp;action=edit_recurrence&amp;recurrence_id=<?php
...
<?php
	if ($events_count > $limit) {
		...
		if ($backward < $events_count)
			echo "<a style='float: left' href='" . get_bloginfo ( 'url' ) . "/wp-admin/admin.php?page=events-manager/events-manager.php&amp;scope=$scope&offset=$backward'>&lt;&lt;</a>";
		if ($forward >= 0)
			echo "<a style='float: right' href='" . get_bloginfo ( 'url' ) . "/wp-admin/admin.php?page=events-manager/events-manager.php&amp;scope=$scope&offset=$forward'>&gt;&gt;</a>";
		echo "</div>";
	}
	?>
...
function dbem_event_form($event, $title, $element) {
	...
	$_GET ['action'] == "edit_recurrence" ? $form_destination = "admin.php?page=events-manager/events-manager.php&amp;action=update_recurrence&amp;recurrence_id=" . $element : $form_destination = "admin.php?page=events-manager/events-manager.php&amp;action=update_event&amp;event_id=" . $element;
		...
		name="recurrence_byday">
			...
			if (! $event ['recurrence_id']) {
				...
				?>
									...
									<a
		href="<?php
				bloginfo ( 'wpurl' )?>/wp-admin/admin.php?page=events-manager/events-manager.php&amp;action=edit_recurrence&amp;recurrence_id=<?php
				echo $recurrence ['recurrence_id'];
				?>">
									<?php
				_e ( 'Reschedule', 'dbem' );
