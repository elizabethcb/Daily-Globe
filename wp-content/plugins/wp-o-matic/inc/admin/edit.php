          ...
          <div id="edit_feed">
            <?php if(isset($data['feeds']['edit'])): ?>
              ...
              <div class="inlinetext required">
                ...
                <?php echo input_tag('campaign_feed[edit]['. $id .']', $feed, 'disabled=disabled class=input_text id=campaign_feed_edit_' . $id) ?> <br />
                <?php $ocamps = $this->getCampaigns( array( 'fields' => 'title, id', 'type' => 'ARRAY_A' ) );
                $camps = array(0 => 'Somewhere Else');
                foreach ($ocamps as $o) {
                	$camps[$o['id']] = $o['title'];
                }
                	?>
                <!--<pre> <?php print_r ($camps);?></pre>-->
                <?php 
                	echo label_for( 'campaign_feed_move_' . $id, "Approve and Move to Here..." );
					echo select_tag( 'campaign_feed[move][' . $id . ']', options_for_select($camps) ); 
                	echo checkbox_tag('campaign_feed[delete]['.$id.']', 1, (isset($data['feeds']['delete']) && _data_value($data['feeds']['delete'], $id)), 'id=campaign_feed_delete_' . $id) ?> <label for="campaign_feed_delete_<?php echo $id ?>" class="delete_label">Delete ?</label>
              </div>
              <?php endforeach ?>
            <?php endif ?>
